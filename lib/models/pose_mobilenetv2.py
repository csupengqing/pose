...
class MobiletNetV2Block(nn.Module):
    expansion = 1
    def __init__(self, input_c, output_c, stride=1, downsample=None, expand_ratio=6):
        super(MobiletNetV2Block, self).__init__()
        assert stride in [1, 2]
        expand_dim = round(input_c * expand_ratio)
        # basic block
        self.blcok_type = stride == 1 and input_c == output_c
        
        if expand_ratio == 1:
            self.conv = nn.Sequential(
                # dw
                nn.Conv2d(expand_dim, expand_dim, kernel_size=3, stride=stride, padding=1, groups=expand_dim, bias=False),
                nn.BatchNorm2d(expand_dim),
                nn.ReLU6(inplace=True),
                # pw-linear
                nn.Conv2d(expand_dim, output_c, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(output_c)
            )
        else:
            self.conv = nn.Sequential(
                # pw
                nn.Conv2d(input_c, expand_dim, kernel_size=1, stride=1, padding=0, bias=False),
                nn.BatchNorm2d(expand_dim),
                nn.ReLU6(inplace=True),
                # dw
                nn.Conv2d(expand_dim, expand_dim, kernel_size=3, stride=stride, padding=1, groups=expand_dim, bias=False),
...
class Stem(nn.Module):
    def __init__(self, cfg, in_channels, stem_channels, out_channels, expand_ratio=6):
        super().__init__()
        ...
        #MobileBlock
        expand_channels = round(stem_channels * expand_ratio)
        self.conv = nn.Sequential(
        # pw
        nn.Conv2d(stem_channels, expand_channels, kernel_size=1, stride=1, padding=0, bias=False),
        nn.BatchNorm2d(expand_channels),
        nn.ReLU6(inplace=True),
        # dw
        nn.Conv2d(expand_channels, expand_channels, kernel_size=3, stride=2, padding=1, groups=expand_channels, bias=False),
        nn.BatchNorm2d(expand_channels),
        nn.ReLU6(inplace=True),
        # pw-linear
