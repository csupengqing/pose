...
class PoseHighResolutionNet(nn.Module):
    def __init__(self, cfg, **kwargs):
        ...
        self.upsample3 = nn.Sequential(
            nn.Conv2d(128,16,1,1,0,bias=False),
            nn.BatchNorm2d(16),
            nn.Upsample(scale_factor=8,mode="nearest")
        )
        self.upsample4 = nn.Sequential(
            nn.Conv2d(256,16,1,1,0,bias=False),
            nn.BatchNorm2d(16),
    ...
    def forward(self, x):
        ...
        #fuse
        y1 = self.conv(x1)
        y2 = self.upsample1(y_list[0])
        y3 = self.upsample2(y_list[1])
        y4 = self.upsample3(y_list[2])
        y5 = self.upsample4(y_list[3])

        y = y1 + y2 + y3 + y4 + y5
        y = self.relu(y)

        x_ = self.final_layer(y)
